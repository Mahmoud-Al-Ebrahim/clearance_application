...
class CacheHelper {
  ...
  static init() async {
    ...
    if(getEnterAppCount()==null){
      saveEnterAppCount(1);
    }
    else{
      saveEnterAppCount((int.parse(getEnterAppCount()!)+1));
    }
  }
...
Future<void> saveCacheServerUrl(String? token) async {
  CacheHelper.saveData(key: 'server_url', value: token);
}

String? getCachedServerUrl() {
  return CacheHelper.getData(key: 'server_url');
}
...
//enter_app
Future<void> saveEnterAppCount(int count) async {
  CacheHelper.saveData(key: 'open_app', value: count.toString());
}

String? getEnterAppCount() {
  return CacheHelper.getData(key: 'open_app');
}
...
Future<void> clearAllCacheWithoutTheme() async {
  ...
  String defaultDialCode=getCachedPhoneDialCode()!;
  CacheHelper.clearAllData();
  CacheHelper.init();
  saveCachePhoneDialCode(defaultDialCode);
