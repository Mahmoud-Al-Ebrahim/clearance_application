...
class SearchCubit extends Cubit<SearchStates> {
  ...
  void initSearch(String? query){
    if(previousSearchText!=null){
      if(previousSearchText!.trim()== query?.trim()){
        return ;
      }
    }
    ...
    searchAutoCompleteModel=null;
  }

  void resetSearchCubitFields(){
    pageNumber=0;
    products=[];
    filteredProductsModel=null;
    searchAutoCompleteModel=null;
  ...
  Future<void> getSearchResults(String searchText, String customCateId,{bool withPagination=true}) async {
    inSearchProcess = true;
    initSearch(searchText);
    if(!withPagination) {
      resetSearchCubitFields();
    }
    pageNumber++;
