...
import 'package:clearance/modules/main_layout/sub_layouts/account/wallet/return_exchange_stepper.dart';
        ...
        isRelatedSoNavigateAndFinish
            ? navigateToAndFinish(
                ...
                ProductDetailsLayout(
                  ...
                  fromExchange: forExchange ?? false,
          ...
          ClipRRect(
              ...
              child: SizedBox(
                ...
                child: DefaultButton(
                  ...
                  onClick: () {
                    MainCubit.get(context).navigateToTapMethod('0');
                  },
                ),
              ))
        ],
...
class PhoneInputField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    logg('cached dial :' + (getCachedPhoneDialCode() ?? ''));
    logg('cached code :' + (getCachedPhoneCode() ?? ''));
    var localizationStrings = AppLocalizations.of(context);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                          ...
                          childWidget: Padding(
                            ...
                            child: InternationalPhoneNumberInput(
                              ...
                              validator: (String? val) {
                                ...
                                } else if (double.tryParse(val.replaceAll(
                                        RegExp(r'[^0-9]'), '')) ==
                                    null) {
                                  isValidate.value = false;
                              ...
                              onInputChanged: onInputChanged == null
                                  ? (PhoneNumber value) {
                                      ...
                                      phone = _replaceArabicNumber(phone);
                                      accountCubit.updatePhoneNum(PhoneNumber(
                                          phoneNumber: phone,
                                      ...
                                      saveCachePhoneNumber(phone.substring(sz));
                                    }
                                  : (PhoneNumber value) {
                                      String myPhone = value.phoneNumber ?? '';
                                      myPhone = _replaceArabicNumber(myPhone);
                                      PhoneNumber p = PhoneNumber(
                                          phoneNumber: myPhone,
                                          isoCode: value.isoCode,
                                          dialCode: value.dialCode);
                                      onInputChanged!.call(p);
                                      phone = p;
                                    },
                              locale: locale,
...
class _DefaultImageState extends State<DefaultImage> {
  ...
  @override
  Widget build(BuildContext context) {
    currentUrl = widget.backGroundImageUrl!;
        ...
        builder: (context, count, _) {
          return Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(widget.withoutRadius != null
                    ? 0.0
                    : widget.radius ?? 5.0.sp),
                border: Border.all(
                    width: 1.0, color: widget.borderColor ?? Colors.black),
              ),
              child: ClipRRect(
                borderRadius: BorderRadius.circular(widget.withoutRadius != null
                    ? 0.0
  ...
  Widget build(BuildContext context) {
    ...
    return LikeButton(
      ...
      onTap: (bool isLiked) async {
        setCurrentAction(buttonName: '(AddTo/RemoveFrom)FavoriteButton');
                                ...
                                onTap: () {
                                  navigateToWithNavBar(
                                      ...
                                      SubSubCategoryLayout(
                                        cateId: categoriesModel.data![index].id!
                                            .toString(),
                                      ),
...
class ProductsGridView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ConditionalBuilder(
      ...
      fallback: (context) => showNoProductsMessage
          ? SizedBox(
              height: 115.h,
              child: const Center(
...
class SearchAppBarForExchange extends StatelessWidget {
  const SearchAppBarForExchange(
      {Key? key,
        this.customCateId,
    ...
    return AppBar(
      ...
      title: Column(
        ...
        children: [
          ...
          5.verticalSpace,
          ReturnAndExchangeStepper(stepCount:stepCount ,currentPage:  currentPage),
          5.verticalSpace,
          DefaultContainer(
...
class CustomSearchDelegate extends SearchDelegate<String> {
  ...
  bool showResultsForBanner = false;

  @override
  set query(String value) {
    // TODO: implement query
    showResultsForBanner = defaultQuery != null;
    super.query = defaultQuery ?? value;
    defaultQuery = null;
  }

  @override
  ThemeData appBarTheme(BuildContext context) {
    if (showResultsForBanner) {
      showResults(context);
      showResultsForBanner = false;
    }
  ...
  Widget buildResults(BuildContext context) {
    ...
    !searchCubit.inSearchProcess && query!=searchCubit.previousSearchText
        ? {
      logg('here'),
      searchCubit.getSearchResults(query, customCateId ?? '', withPagination: false)
    }
        : null;
...
class DefaultAppBarWithTitleAndBackButton extends StatelessWidget {
  const DefaultAppBarWithTitleAndBackButton(
      {Key? key, this.transparent, required this.title})
      : super(key: key);
  final bool? transparent;
  final String title;
...
class DefaultAppBarWithTitleAndBackButtonAndSteps extends StatelessWidget {
  const DefaultAppBarWithTitleAndBackButtonAndSteps(
      {Key? key, this.transparent, required this.title ,required this.stepCount, required this.currentPage})
      : super(key: key);
  final bool? transparent;
  final String title;
  final int stepCount;
    ...
    return RichText(
      text: TextSpan(
        ...
        children: <TextSpan>[
          TextSpan(
            text: price.split(' ')[0],
            style: mainStyle(18.0, FontWeight.w900,
                color: priceColor, fontFamily: 'poppins'),
          ),
          TextSpan(
            text: ' ',
