...
class AuthCubit extends Cubit<AuthStates> {
  ...
  bool requestRegisterGuest=false;
  bool requestCheckUser=false;
  ...
  Future<void> socialMediaUserLogin({
    ...
    mainCubit.getDeviceId().then((value) async {
      await MainDioHelper.postData(url: socialLoginEnd, data: {
        ...
        checkUserAuth(context).then(
            (value) => navigateToAndFinishUntil(context, const MainAppMaterialApp()));
  ...
  Future<void> guestLogin({
    ...
    if(requestRegisterGuest){
      return ;
    }
    requestRegisterGuest=true;
    var mainCubit = MainCubit.get(context);
    verifiedNow=false;
    ...
    await MainDioHelper.postData(url: guestRegisterEnd, data: {
      ...
      if (userSignUpModel != null) {
        userCacheProcess(userSignUpModel!).then((value) => checkUserAuth(context).then(
            (value) =>
                isFirstRun ? navigateToAndFinishUntil(context, const RouteEngine()) : Navigator.popUntil(context, (route) => route.isFirst)));
  ...
  Future<void> userEmailLogin({
    ...
    mainCubit.getDeviceId().then((value) async {
      await MainDioHelper.postData(
        ...
        if (userSignUpModel != null) {
          userCacheProcess(userSignUpModel!).then((value) => checkUserAuth(context)
              .then((value) =>
  ...
  Future<void> userRegister({
    ...
    required int sendUsingWhatsapp,
    ...
    mainCubit.getDeviceId().then((value) async {
      await MainDioHelper.postData(url: newRegisterEnd, data: {
        ...
        'user_type': '1',
        'phone': phone,
        'is_via_whatsapp': sendUsingWhatsapp,
  ...
  Future<void> checkUserAuth(BuildContext context) async {
    ...
    if(requestCheckUser || requestRegisterGuest){
      return ;
    }
    requestCheckUser=true;
    ...
    if (token != null) {
      await MainDioHelper.getData(url: userInfoEnd, token: token).then((value) {
        ...
        MainCubit.get(context).initial(isLogout: false , context: context);
        isUserAuthChecked = true;
        requestCheckUser=false;
        ...
        if(error is DioError) {
          emit(CheckingAuthStateError(
              requestRegisterGuest: (error.response?.statusCode==401)
          ));
        }
        else{
          emit(CheckingAuthStateError());
