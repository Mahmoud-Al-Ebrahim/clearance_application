...
class _MainLayoutState extends State<MainLayout> {
  ...
  @override
  void initState() {
    setCurrentScreen(screenName: 'MainLayout');
  ...
  Widget build(BuildContext context) {
    ...
    mainCubit.navigateToTap = (String index) {
      _controller!.jumpToTab(int.parse(index));
    };
    ...
    return BlocConsumer<MainCubit, MainStates>(
      ...
      builder: (context, state) => PersistentTabView(
        ...
        onItemSelected: (index) {
          ...
          ProductDetailsLayout.popProductDetails?.call();
          if (index == 0) {
      ...
      icon: BlocBuilder<CartCubit, CartStates>(
          ...
          builder: (context, state) {
            return badges.Badge(
              ...
              showBadge:
                  (CartCubit.get(context).cartModel?.data?.cart?.length ?? 0) >
                      0,
              badgeContent: Text(
                  CartCubit.get(context)
                          .cartModel
      ...
      icon: BlocBuilder<AccountCubit, AccountStates>(
          ...
          builder: (context, state) {
            return badges.Badge(
              position: badges.BadgePosition.topStart(start: -5, top: -5),
              showBadge:
                  (AccountCubit.get(context).wishListProducts?.length ?? 0) > 0,
              badgeContent: Text(
