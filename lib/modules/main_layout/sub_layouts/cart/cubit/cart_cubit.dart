...
void disableCouponStillAvailable(){
  ...
  required bool updateAllList,
    ...
    await MainDioHelper.getData(
        url: getCartWithEstimatedTaxEnd, token: getCachedToken())
        .then((value) {
      logg('got cart');
      logg(value.toString());
  ...
  Future<void> removeItem(String itemId , {int? cartPosition , BuildContext? context} )async{
    currentRemovingItemId=itemId;


    if(cartPosition != null){
      itemId = cartModel!.data!.cart![cartPosition].id.toString();
      currentRemovingItemId=itemId;
    }
    ...
    await MainDioHelper.postData(
      ...
      if(cartPosition != null) {
        PaymentCubit
            ...
            .data!
            .removeWhere((element) => element.cartPosition == cartPosition);
        PaymentCubit.get(context).emit(CheckAvailabilitySuccess(openVerificationOtp: false));
      }
  ...
  Future<void> applyCoupon(String couponCode)async{
    ...
    await MainDioHelper.getData(
        url: applyCouponCodeEnd+'?code=$couponCode', token: getCachedToken(),
    )
        .then((value) {
