...
class ProductDetailsCubit extends Cubit<ProductDetailsStates> {
  ...
  Map<String,bool> variationsSelected ={};
  String selectVariantMessage="";
  ...
  void addSelectedChoice(BuildContext context, int id, String choiceTitle,
      String choiceName, String choiceValue , bool popScreen) {
    logg('adding choices');
    variationsSelected[choiceTitle]=true;
    try {
    ...
    updateProductDetails(skuString!);
    emit(ChoiceChangedState(popScreen: popScreen));
  }
  ...
  void increaseQty() {
    if (!getAllVariantsSelected()) {
      emit(PleaseChooseVariantState(
        message: selectVariantMessage
      ));
      return;
    }
  ...
  Future<void> getProductDetails(
    ...
    await MainDioHelper.getData(
      ...
      if (productDetailsModel!.data!.choiceOptions?.isNotEmpty ?? false) {
        logg('choice options is not empty');
        for (int i = 0; i < productDetailsModel!.data!.choiceOptions!.length; i++) {
          ...
          skuLis!.add(productDetailsModel!.data!.choiceOptions![i].options![0]);
            ...
            variationsSelected[productDetailsModel!.data!.choiceOptions![i].title!] = false;
        }
  ...
  Future<void> addToCart(
    ...
    if (!getAllVariantsSelected()) {
      emit(PleaseChooseVariantState(message: selectVariantMessage , fromBuyNowButton: buyNow , quantity:quantity));
      return;
    }
  ...
  bool getAllVariantsSelected(){
    bool allVariantsSelected=true;
    variationsSelected.forEach((key, value) {
      allVariantsSelected&=value;
      if(!value){
        selectVariantMessage=key;
      }
    });
