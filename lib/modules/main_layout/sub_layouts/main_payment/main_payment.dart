...
class PaymentMainLayout extends StatefulWidget {
  const PaymentMainLayout({Key? key}) : super(key: key);
  static String routeName = 'PaymentMainLayoutRoute';

  @override
  State<PaymentMainLayout> createState() => _PaymentMainLayoutState();
}

class _PaymentMainLayoutState extends State<PaymentMainLayout> {

  @override
  void initState() {
    setCurrentScreen(screenName: 'PaymentMainLayout');
  ...
  Widget build(BuildContext context) {
    lastScreen = 'PaymentMainLayout';
    var cartCubit = CartCubit.get(context)..disableCouponStillAvailable();
            ...
            BlocConsumer<PaymentCubit, PaymentsStates>(
              listener: (context, state) {
                if (state is CheckAvailabilitySuccess) {
                  if (state.openVerificationOtp) {
                    paymentTransaction(paymentCubit, localizationStrings,
                        notesController.text, mainCubit, context);
                  }
                }
                if (state is CheckAvailabilityFailure && state.fromPayment) {
                  showProductAvailabilityError(context);
                }
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    ...
                                    child: DefaultButton(
                                      title:
                                          localizationStrings.find_more_deals,
                                      onClick: () {
                                        Navigator.pop(context);
                                        mainCubit.navigateToTapMethod('0');
                                      },
                                    ),
                                  )
                                ],
              ...
              builder: (context, state) {
                ...
                return BlocBuilder<CartCubit, CartStates>(
                  builder: (context, state) => cartCubit.cartModel == null
                      ? const Center(child: DefaultLoader())
                      : ConditionalBuilder(
                          condition:
                              cartCubit.cartModel!.data!.cart!.isNotEmpty,
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        children: <
                                                                          ...
                                                                          ConditionalBuilder(
                                                                            condition:
                                                                                mainCubit.addressesModel != null,
                                                                            builder:
                                                                                (context) {
                                                                              List<Address> addresses = mainCubit.addressesModel!.data!;
                                                                              return addresses.isNotEmpty
                                                                                  ? ConditionalBuilder(
                                                                                      condition: mainCubit.addressesModel!.data!.isNotEmpty,
                                                                                          ...
                                                                                          child: Column(
                                                                                            children: [
                                                                                              ListView.separated(
                                                                                                ...
                                                                                                itemBuilder: (context, index) => AddressItem(notInAddressListJustForView: true, inBottomModalSheet: true, localizationStrings: localizationStrings, address: addresses[index]),
                                                                          ...
                                                                          DefaultButton(
                                                                              onClick: () {
                                                                                setCurrentAction(buttonName: 'AddNewAddressButton');
                                                                                navigateToWithNavBar(context, const AddNewAddress(), AddNewAddress.routeName);
                                                                              },
                                  ...
                                  DefaultContainer(
                                    ...
                                    childWidget: Column(
                                      children: [
                                        ...
                                        BlocConsumer<PaymentCubit,
                                            PaymentsStates>(
                                          listener: (previous, current) {
                                            if (current
                                                is ErrorSendingPaymentData) {
                                              if (current.error ==
                                                  'user-not-verified') {
                                          ...
                                          builder: (_, state) {
                                            return GestureDetector(
                                              onTap: () {
                                                lastClickedButton =
                                                    'place Order';
                                                setCurrentAction(buttonName: 'PlaceOrderButton');
                                                paymentCubit
  ...
  paymentTransaction(PaymentCubit paymentCubit, localizationStrings,
      String note, MainCubit mainCubit, context) {
    if ((AuthCubit.get(context).userInfoModel!.data!.isVerifiedEmail == 1 ||
            AuthCubit.get(context).userInfoModel!.data!.isVerifiedPhone == 1 ||
            AuthCubit.get(context).verifiedNow) &&
        mainCubit.getDefaultShippingAddressItemValues()!.id == 0) {
      chooseAddress(mainCubit, localizationStrings, context);
      return;
    }
    logg('paymentCubit.selectedPaymentId+=' +
        paymentCubit.selectedPaymentId.toString());
    logg('shipping address selected. navigating to place order');
    if (paymentCubit.selectedPaymentId == '4') {
      paymentCubit.buyWithWallet(
          mainCubit.getDefaultShippingAddressItemValues()!.id.toString(),
          context);
    } else if (paymentCubit.selectedPaymentId == '3') {
      ...
      paymentCubit
          .placeOrder(
              ...
              note,
              context)
          .then((value) => AccountCubit.get(context).getOrders().then((value) =>
      ...
      logg('paymentCubit.selectedPaymentId is 4');
      paymentCubit.getPaymentLinkAndRedirect(
  ...
  chooseAddress(MainCubit mainCubit, localizationStrings, context) {
    logg(mainCubit.defaultShippingAddressId.toString());
              ...
              return Column(
                ...
                children: <Widget>[
                  Align(
                    ...
                    child: InkWell(
                      onTap: () {
                        setCurrentAction(buttonName: 'CloseChooseAddressButton');
                        Navigator.pop(context);
                      },
                      child: Padding(
                        padding:
                            EdgeInsetsDirectional.fromSTEB(0, 8.h, 14.h, 0),
                        child: Text(
                          'X',
                  ...
                  Padding(
                    ...
                    child: DefaultButton(
                        onClick: () {
                          lastClickedButton = 'add new address';
                          Navigator.pop(context);
                          navigateToWithNavBarToAddAddress(context, const AddNewAddress(),
        ...
        children: [
          ...
          ListView.separated(
              ...
              itemBuilder: (context, index) => DeliveryMethodItem(
                    ...
                    isSelected: paymentCubit.deliveryMethods[index].isSelected,
...
class PaymentMethods extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<PaymentMethod?> paymentMethods = [
      ...
      PaymentMethod(
          id: showPaymentWithPostpay ?? true ? 1 : 0,
    ...
    return BlocBuilder<PaymentCubit, PaymentsStates>(
      buildWhen: (p, c) => c is SelectedMethodChanged,
      builder: (context, state) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            ListView.separated(
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                itemBuilder: (context, index) => paymentMethods[index]!.id != 0
                    ? PaymentMethodItem(
                        ...
                        isVisible: true, //cartCubit.cartModel!.data!.hasCod!,
                      )
    ...
    if (isVisible && id == 3.toString()) {
      return GestureDetector(
        onTap: () {
          ...
          setCurrentAction(buttonName: 'ChangeSelectedPaymentMethodToPayCashButton');
      ...
      return GestureDetector(
        onTap: () {
          setCurrentAction(buttonName: 'ChangeSelectedPaymentMethodToPayWithCardButton');
          logg('changing pay  method');
          paymentCubit.changeSelectedPaymentMethodId(id.toString());
        },
        child: Container(
      ...
      return GestureDetector(
        onTap: () {
          setCurrentAction(buttonName: 'ChangeSelectedPaymentMethodToPostPayButton');
          logg('changing pay  method');
          paymentCubit.changeSelectedPaymentMethodId(id.toString());
        },
        child: Container(
      ...
      return GestureDetector(
          onTap: () {
            if(accountCubit.walletModel!=null){
              if(accountCubit.walletModel!.totalWalletBalance! < CartCubit.get(context).cartModel!.data!.total!){
                PaymentCubit.get(context).alertForWallet();
                return;
              }
            }
            setCurrentAction(buttonName: 'ChangeSelectedPaymentMethodToWalletButton');
            paymentCubit.changeSelectedPaymentMethodId(id.toString());
          },
          child: Container(
            color: mainGreyColor.withOpacity(0.1),
                  ...
                  builder: (context, state) {
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        3.verticalSpace,
                        Text(
                          localizationStrings!.balance + ': '+
                                  accountCubit.walletModel!
                                  .totalWalletBalanceFormatted
                                  .toString(),
                          style: mainStyle(14.0, FontWeight.w500),
                        ),
                      ],
                    );
                  },
